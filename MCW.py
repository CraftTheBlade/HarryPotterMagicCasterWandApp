...
# Was 30 before. 30 is too low for big spell lists.
# 500 examples per spell gives much more room for 100+ spells.
MAX_EXAMPLES_PER_SPELL = 500
...
BUILTIN_SPELLS = {
    ...
    "Protego": {
        ...
        "seconds": 3,
        "vibration_ms": 220,
        "stay_on_until_next_spell": False,
    },
    "Nox": {
        "mode": "off",
        "color": [0, 0, 0],
...
class WandCore:
    def __init__(self, ui_queue):
        ...
        # New: can be toggled from GUI.
        self.auto_learn_successful_casts = True
        ...
        self.scheduled_light_off_task = None
    ...
    def set_auto_learn(self, enabled):
        self.auto_learn_successful_casts = bool(enabled)
        self.log(f"Auto-learn successful casts {'ON' if enabled else 'OFF'}")
        ...
        try:
            ...
            for spell, cfg in loaded.items():
                ...
                fixed[spell] = {
                    ...
                    "vibration_ms": int(cfg.get("vibration_ms", 0)),
                    # New field. Old custom spells get default timed-off behavior.
                    "stay_on_until_next_spell": bool(cfg.get("stay_on_until_next_spell", False)),
                }

            self.custom_spells = fixed
    ...
    async def disconnect(self):
        ...
        try:
            ...
            self.cancel_scheduled_light_off()
            if self.client and self.client.is_connected:
                try:
                    await self.led_off()
                except Exception:
    ...
    def cancel_scheduled_light_off(self):
        if self.scheduled_light_off_task and not self.scheduled_light_off_task.done():
            self.scheduled_light_off_task.cancel()
        self.scheduled_light_off_task = None

    def schedule_light_off_if_needed(self, config):
        self.cancel_scheduled_light_off()

        stay_on = bool(config.get("stay_on_until_next_spell", False))
        mode = config.get("mode", "all")
        seconds = float(config.get("seconds", 5))

        if stay_on or mode == "off":
            return
        ...
        self.scheduled_light_off_task = asyncio.create_task(delayed_off())
    ...
    async def effect_tip(self, color, seconds, vibration_ms=0):
        ...
        await self.led_off()
        await self.led_color(0x00, r, g, b)
        # Light timing is handled by schedule_light_off_if_needed.
    ...
    async def effect_pulse(self, color, seconds, vibration_ms=0, stay_on=False):
        await self.vibrate(vibration_ms)
        ...
        # For stay-on spells, pulse a few times then leave the final color on.
        pulse_time = min(max(1.0, float(seconds)), 8.0)
        end_time = asyncio.get_event_loop().time() + pulse_time
        pulse_high = False
    ...
    def run_spell_action(self, spell: str, source="AI"):
        ...
        # New spell cancels old stay-on/timed-off behavior.
        self.cancel_scheduled_light_off()
        ...
        if mode == "tip":
            ...
            self.start_light_task(self.effect_pulse(color, seconds, vibration_ms, stay_on=stay_on))
        elif mode == "off":
            async def off_effect():
                await self.vibrate(vibration_ms)
                await self.keep_leds_off(seconds=seconds)
        ...
        # New: if not stay-on, schedule turn-off after chosen seconds.
        self.schedule_light_off_if_needed(config)
    ...
    def add_training_example(self, spell_name, samples=None, silent=False):
        ...
        # New larger max. Keeps newest examples when extremely full.
        while len(self.training_data[spell_name]) > MAX_EXAMPLES_PER_SPELL:
            self.training_data[spell_name].pop(0)

        self.save_training()

        if silent:
            self.log(
                f"Auto-learned successful cast for {spell_name}. "
                f"Examples: {len(self.training_data[spell_name])}/{MAX_EXAMPLES_PER_SPELL}"
            )
        else:
            self.log(
                f"Saved last motion as {spell_name}. "
                f"Examples: {len(self.training_data[spell_name])}/{MAX_EXAMPLES_PER_SPELL}"
            )
    ...
    def get_trained_spell_count(self):
        return sum(
            ...
            if spell != "Unknown" and spell in self.all_spell_configs() and examples
        )
    ...
    def classify_motion(self, samples):
        ...
        for spell, examples in self.training_data.items():
            if spell == "Unknown":
                continue
            if spell not in self.all_spell_configs():
                continue
            if not examples:
                continue
        ...
        raw_scores = []
        for spell, distance in spell_distances:
            score = 1.0 / ((distance * distance) + 0.001)
            raw_scores.append((spell, score, distance))

        total = sum(score for _, score, _ in raw_scores)

        results = []
        for spell, score, distance in raw_scores:
            percent = round((score / total) * 100)
            results.append((spell, percent, distance))

        best_spell = results[0][0]
        return best_spell, results
    ...
    def maybe_auto_cast(self, samples):
        ...
        if should_cast:
            ...
            # New: when the AI successfully casts, this movement is added as another
            # example for the same spell. It makes repeated correct movements improve the AI.
            if self.auto_learn_successful_casts:
                self.add_training_example(best_spell, samples=samples, silent=True)
        else:
            ...
            if len(data) >= 4 + text_len:
                try:
                    ...
                    if spell:
                        ...
                        # Official recognizer being correct is also a good training example.
                        if self.auto_learn_successful_casts and self.last_motion_samples:
                            self.add_training_example(spell, self.last_motion_samples, silent=True)
    ...
    def add_custom_spell(self, name, mode, color, seconds, vibration_ms, stay_on_until_next_spell):
        ...
        self.custom_spells[name] = {
            ...
            "stay_on_until_next_spell": bool(stay_on_until_next_spell),
        }
        ...
        # Important fix:
        # This event refreshes BOTH Training and Manual Effects lists in the GUI.
        self.ui("spells_changed", None)

        if stay_on_until_next_spell:
            timing = "stays on until another spell is cast"
        else:
            timing = f"turns off after {seconds} seconds"

        self.log(f"Added custom spell: {name} ({timing})")
...
class AddSpellDialog(tk.Toplevel):
    def __init__(self, parent, on_save):
        ...
        # New custom-spell light timing choice.
        self.light_timing_var = tk.StringVar(value="timed")

        frm = ttk.Frame(self, padding=12)
        frm.grid(row=0, column=0, sticky="nsew")

        ttk.Label(frm, text="Spell name:").grid(row=0, column=0, sticky="w", pady=4)
        ttk.Entry(frm, textvariable=self.name_var, width=34).grid(row=0, column=1, sticky="ew", pady=4)

        ttk.Label(frm, text="Light movement:").grid(row=1, column=0, sticky="w", pady=4)
        ttk.Combobox(frm, textvariable=self.mode_var, values=LIGHT_MODES, state="readonly", width=31).grid(row=1, column=1, sticky="ew", pady=4)
        ...
        ttk.Entry(color_row, textvariable=self.color_text_var, width=20).pack(side="left")
        ttk.Button(color_row, text="Pick", command=self.pick_color).pack(side="left", padx=5)

        ttk.Label(frm, text="Light timing:").grid(row=3, column=0, sticky="nw", pady=4)
        timing_frame = ttk.Frame(frm)
        timing_frame.grid(row=3, column=1, sticky="w", pady=4)

        ttk.Radiobutton(
            timing_frame,
            text="Stay on until a new spell gets cast",
            variable=self.light_timing_var,
            value="stay",
            command=self.update_timing_state,
        ).pack(anchor="w")

        ttk.Radiobutton(
            timing_frame,
            text="Turn off after a time",
            variable=self.light_timing_var,
            value="timed",
            command=self.update_timing_state,
        ).pack(anchor="w")

        ttk.Label(frm, text="Turn-off time:").grid(row=4, column=0, sticky="w", pady=4)
        seconds_row = ttk.Frame(frm)
        seconds_row.grid(row=4, column=1, sticky="ew", pady=4)
        self.seconds_entry = ttk.Entry(seconds_row, textvariable=self.seconds_var, width=12)
        self.seconds_entry.pack(side="left")
        ttk.Label(seconds_row, text="seconds").pack(side="left", padx=5)

        ttk.Checkbutton(frm, text="Use vibration", variable=self.use_vibration_var).grid(row=5, column=0, sticky="w", pady=4)
        vibration_row = ttk.Frame(frm)
        vibration_row.grid(row=5, column=1, sticky="ew", pady=4)
        ttk.Entry(vibration_row, textvariable=self.vibration_var, width=12).pack(side="left")
        ttk.Label(vibration_row, text="ms").pack(side="left", padx=5)
    ...
    def update_timing_state(self):
        if self.light_timing_var.get() == "stay":
    ...
    def save(self):
        try:
            ...
            stay_on = self.light_timing_var.get() == "stay"
            ...
            self.on_save(name, mode, color, seconds, vibration_ms, stay_on)
            self.destroy()
...
class MagicCasterGUI(tk.Tk):
    def __init__(self):
        ...
        self.training_limit_var = tk.StringVar(value=f"Training limit: {MAX_EXAMPLES_PER_SPELL} per spell")

        self.build_ui()
        self.refresh_all_spell_buttons()
    ...
    def build_ui(self):
        ...
        ttk.Checkbutton(
            controls,
            text="Auto-learn successful casts",
            variable=self.auto_learn_var,
            command=self.toggle_auto_learn,
        ).pack(side="left", padx=8)

        ttk.Checkbutton(
            controls,
            text="Show training",
            variable=self.training_visible,
        ...
        ttk.Label(info, textvariable=self.training_limit_var).grid(row=3, column=0, sticky="w", padx=5, pady=2)

        self.manual_frame = ttk.LabelFrame(root, text="Manual Effects", padding=10)
        self.manual_frame.pack(fill="x", pady=8)

        manual_top = ttk.Frame(self.manual_frame)
        manual_top.pack(fill="x", pady=(0, 6))
        ttk.Button(manual_top, text="Add New Spell", command=self.add_spell).pack(side="right", padx=3)

        self.manual_buttons_frame = ttk.Frame(self.manual_frame)
        self.manual_buttons_frame.pack(fill="x")

        self.training_frame = ttk.LabelFrame(root, text="Training", padding=10)
        self.training_frame.pack(fill="x", pady=8)

        self.training_buttons_frame = ttk.Frame(self.training_frame)
        self.training_buttons_frame.pack(fill="x")
    ...
    def refresh_all_spell_buttons(self):
        self.refresh_manual_buttons()
        self.refresh_training_buttons()

    def refresh_manual_buttons(self):
        # Important fix:
        # Manual list is now dynamic, so custom spells appear here too.
        for child in self.manual_buttons_frame.winfo_children():
            child.destroy()

        spells = list(self.core.all_spell_configs().keys())

        if not spells:
            ttk.Label(self.manual_buttons_frame, text="No spells yet.").grid(row=0, column=0, sticky="w")
            return
        ...
        for i, spell in enumerate(spells):
            ...
            col = i % 4
            btn = ttk.Button(
                self.manual_buttons_frame,
                text=spell,
                command=lambda s=spell: self.manual_effect(s),
            )
            btn.grid(row=row, column=col, padx=4, pady=4, sticky="ew")
            self.manual_buttons_frame.grid_columnconfigure(col, weight=1)

    def refresh_training_buttons(self):
        for child in self.training_buttons_frame.winfo_children():
            child.destroy()
        ...
        for i, spell in enumerate(spells):
            ...
            text = f"Train {spell} ({count}/{MAX_EXAMPLES_PER_SPELL})"
            btn = ttk.Button(
                self.training_buttons_frame,
                text=text,
                command=lambda s=spell: self.train_spell(s),
            )
            row = i // 3
            col = i % 3
            btn.grid(row=row, column=col, padx=4, pady=4, sticky="ew")
            self.training_buttons_frame.grid_columnconfigure(col, weight=1)

        last_row = (len(spells) // 3) + 1

        unknown_count = len(self.core.training_data.get("Unknown", []))
        ttk.Button(
            self.training_buttons_frame,
            text=f"Train Unknown / Bad Motion ({unknown_count}/{MAX_EXAMPLES_PER_SPELL})",
            command=lambda: self.train_spell("Unknown"),
        ).grid(row=last_row, column=0, padx=4, pady=4, sticky="ew")

        ttk.Button(
            self.training_buttons_frame,
            text="Show Counts",
            command=self.show_counts,
        ).grid(row=last_row, column=1, padx=4, pady=4, sticky="ew")

        ttk.Button(
            self.training_buttons_frame,
            text="Add New Spell",
            command=self.add_spell,
        ).grid(row=last_row, column=2, padx=4, pady=4, sticky="ew")

    def toggle_training(self):
        if self.training_visible.get():
            self.training_frame.pack(fill="x", pady=8, after=self.manual_frame)
        else:
    ...
    def toggle_auto_learn(self):
        self.core.set_auto_learn(self.auto_learn_var.get())
    ...
    def show_counts(self):
        lines = [f"Max examples per spell: {MAX_EXAMPLES_PER_SPELL}", ""]
    ...
    def save_new_spell(self, name, mode, color, seconds, vibration_ms, stay_on_until_next_spell):
        self.core.add_custom_spell(
            name=name,
            mode=mode,
            color=color,
            seconds=seconds,
            vibration_ms=vibration_ms,
            stay_on_until_next_spell=stay_on_until_next_spell,
        )
        # Direct refresh too, so it updates immediately even before queue tick.
        self.refresh_all_spell_buttons()
